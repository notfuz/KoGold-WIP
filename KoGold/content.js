  ...
  (function() {
    try {
      ...
      function createPanel() {
        ...
        // Map embedded theme file -> local asset fallback filename (if you add images to Themes/)
        const KoGold_embeddedThemeAssets = {
        ...
        // Resolve external background URLs in embedded theme edits.
        // Strategy: try fetch -> blob -> dataURL; if that fails, fall back to extension-local asset via getResourcePath.
          ...
          for (const e of clone) {
            ...
            for (const k of Object.keys(e.styles)) {
              ...
              if (m && m[1]) {
                ...
                // fallback: local asset in Themes/ (if user adds files to extension)
                try {
        ...
        async function injectThemeScript(file) {
          // If we have the theme embedded, resolve external backgrounds and apply it
          try {
            if (KoGold_embeddedThemeMap && KoGold_embeddedThemeMap[file]) {
              try {
                const resolved = await resolveEmbeddedThemeBackgrounds(file, KoGold_embeddedThemeMap[file]);
                ...
                // proceed to fallback loading strategies
              }
            }
          } catch (e) {}

          // Fallback: load via script tag, with fetch->blob fallback
          return await new Promise((resolve, reject) => {
            try {
        ...
        (function autoLoadSavedTheme(){
          try {
            // First, try to load serialized theme data (contains resolved background data URLs)
            const data = localStorage.getItem('kogold:selectedThemeData');
            if (data) {
              try {
                const parsed = JSON.parse(data);
                if (parsed && Array.isArray(parsed)) {
                  window.CONTAINER_EDITS = parsed;
                  if (window.KoGold_applyContainerEdits) window.KoGold_applyContainerEdits();
                  return;
                }
              } catch (e) {}
